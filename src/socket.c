...
bool set_opts(int socket) {
  ...
  t_socketop options[] = {
    ...
    // it may delay the ACK response by up to 500ms, and we don't want that because 
    // slow bad fast good
  ...
  for(int i = 0; i < sizeof(options)/sizeof(t_socketop); i++)
    if(setsockopt(socket, options[i][0], options[i][1], 
          &flag, sizeof(flag)) < 0)
      return false;
...
void socket_con(struct evconnlistener *listener,
...
bool start_socket(app_t* app, char* addr, int port) {
  ...
  struct evconnlistener *listener = evconnlistener_new_bind(
      base, socket_con, NULL, LEV_OPT_CLOSE_ON_FREE|LEV_OPT_REUSEABLE|LEV_OPT_THREADSAFE,
      -1, (struct sockaddr*)&address, sizeof(address));
  if(NULL == listener){
    errno = ListenFailed;
    return ret;
  }
