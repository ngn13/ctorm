...
char value_valid[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_ :;.,\\/\"'?!(){}[]@<>=-+*#$&`|~^%";
char url_valid[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-._~:/?#[]@!$&'()*+,;%=";
char name_valid[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_";
char* versions[] = {"HTTP/1.1", "HTTP/1.0"};

enum parse_st parse_order[] = {
  METHOD, SPACE, PATH, VERSION, 
  NEWLINE, NAME, VALUE,
  NEWLINE,
  BODY,
};
...
handle_st handle_request(req_t* req, int s){
  size_t i = 0, indx = 0, parse_max = 
    sizeof(parse_order)/sizeof(enum parse_st)-1;
  ...
  while(recv(s, cur, 1, MSG_WAITALL)){
    ...
    if(P_OK == r){
      debug("(Socket %d) Parsing the next section (%d/%d)", 
          s, i, parse_max);
      r = P_FAIL;
      indx = 0;
      i++;
  ...
  if(!done){
    debug("(Socket %d) %s, most likely the connection died", 
        s, strerror(errno));
    return H_CONFAIL;
  }
...
parse_res parse_urldata(pairs_t* pairs, char* rest, int size){
  ...
  for(int c = 0; c < restsize; c++){
    ...
    if(!iskey && rest[c] == '&'){
      ...
      urldecode(key);
      urldecode(value);
      pair_add(pairs, strdup(key), true);
      pair_set(pairs, strdup(value));
      continue;
    }
